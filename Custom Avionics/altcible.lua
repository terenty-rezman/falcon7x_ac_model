...
defineProperty("cabalt", globalPropertyf(
...
defineProperty("RATfail",
               globalPropertyi("sim/operation/failures/ram_air_turbine_on"))
createProp("sim/custom/xap/distance", "float")
defineProperty("distance", globalPropertyf("sim/custom/xap/distance"))
defineProperty("overrideaccel",
               globalPropertyi("sim/operation/override/override_throttles"))
defineProperty("throuse0",
               globalPropertyf("sim/flightmodel/engine/ENGN_thro_use[0]"))
defineProperty("throuse1",
               globalPropertyf("sim/flightmodel/engine/ENGN_thro_use[1]"))
defineProperty("throuse2",
               globalPropertyf("sim/flightmodel/engine/ENGN_thro_use[2]"))
defineProperty("throttle", globalPropertyf(
                   "sim/cockpit2/engine/actuators/throttle_ratio_all"))
defineProperty("altavion", globalPropertyf(
                   "sim/cockpit2/gauges/indicators/altitude_ft_pilot"))
defineProperty("tempsat", globalPropertyf(
...
defineProperty("actmonth",
               globalPropertyi("sim/cockpit2/clock_timer/current_month"))
defineProperty("rolldeg", globalPropertyf(
                   "sim/cockpit2/gauges/indicators/roll_AHARS_deg_pilot"))
defineProperty("rollratio",
               globalPropertyf("sim/flightmodel2/controls/roll_ratio"))
...
defineProperty("pitchratio",
               globalPropertyf("sim/flightmodel2/controls/pitch_ratio"))
defineProperty("navlum",
               globalPropertyi("sim/cockpit2/switches/navigation_lights_on"));
defineProperty("beaconlum", globalPropertyi("sim/cockpit2/switches/beacon_on"));
defineProperty("strobelum",
               globalPropertyi("sim/cockpit2/switches/strobe_lights_on"));
defineProperty("genlum1", globalPropertyi(
...
function update()
    ...
    if get(airspeed) < 10 and get(altlimit) < 3 then
        set(rat, 0)
    ...
    if get(dialaltpa) >= 10000 then set(valaltc, get(dialaltpa) / 100) end
    ...
    if viter < 101 then viter = 101 end
    set(vitr, viter)
    ...
    local climgrad = ((0.0000000098 * mtotal ^ 2) - (0.00145835 * mtotal) +
                         59.645) - (altD / 5000) - (sat / 30)
    if climgrad > 15 then climgrad = 15 end
    if climgrad < -15 then climgrad = -15 end
    set(climbgrad, climgrad)
    local pitche = (0.5957 * climgrad) + 10.793 +
                       (((mtotal - 50000) / 4000) * 0.4)
    if pitche > 20 then pitche = 20 end
    if pitche < -20 then pitche = -20 end
    ...
    if get(posivvi) == 2 then set(pathcommand, gvpath) end
    if get(posivvi) < 2 then set(requetevvi, get(actualvvi)) end
    if get(selils) < 4 then set(backcourse, 0) end
    ...
    if n1max > 99.8 then n1max = 99.8 end
    if n1alt < n1max then
        set(maxn1, n1alt)
    else
        set(maxn1, n1max)
    ...
    set(isa, (sat - stdisa))
    set(TBAT, get(TAMB))
    if get(TAMB) < 12 then set(TBAT, 12) end
    local tempisa = 0
    ...
    if flappos <= 9 then V0 = vsf01 end
    if flappos > 9 and flappos <= 20 then V0 = vsf11 end
    if flappos > 20 then V0 = vsf21 end
    ...
    if lldstatu == 0 then set(llsd, 0) end
    if lldstatu == 1 then set(llsd, get(flash)) end
    if lldstatu == 2 then set(llsd, 1) end
    local lldstatug = get(llg)
    if lldstatug == 0 then set(llsg, 0) end
    if lldstatug == 1 then set(llsg, get(flash)) end

    if lldstatug == 2 then set(llsg, 1) end
